const inputEl = (document.getElementsByClassName('app_controls_input'))[0];
const btnEl = (document.getElementsByClassName('app_controls_button'))[0];
...
function createTask(objectData) {
    ...
    root.classList.add('app_list_item');

    if (objectData.isDone) {
        root.classList.add('app_list_item_done');
    }

    const input = document.createElement('input');
    input.classList.add('app_list_checkbox');
    input.type = 'checkbox';
    ...
    txt.classList.add('app_list_item_txt');
    txt.innerText = objectData.text;

    const btn = document.createElement('button');
    btn.classList.add('app_list_btn');
